...
*****************************************************************************
    ...
    Modification History:
        ...
        01/23/98    bel     Created.

		09/14/98	bel		Added birdDisplayErrorDialogs() to allow enabling 
							and disabling of error messages.

		09/14/98	bel		Modified birdGetReading(), birdGetFrame(), and
							birdGetMostRecentFrame() to return TRUE/FALSE 
							instead of a bird error code.  Added 
							birdGetErrorCode() as a means of retrieving the
							error code when one of these funtions fails.
            
		11/05/98	bel		Fixed on/off status of filters for RS232 and ISA
							modes.

		12/17/98	bel		Added "nNumDevices" argument to birdTCPIPWakeUp(),
							since querying for this value does not work on
							wireless systems.

		03/06/99	bel		Added birdGetFastDeviceConfig() and
							birdSetFastDeviceConfig() to speed up
							initialization and reduce errors.

		03/09/99	bel		Optimized birdStartSingleFrame() for speed in
							RS232 mode.

		04/28/99	bel		Modified birdStartFrameStream() and birdGetFrame()
							to ignore bad packets in TCP/IP mode.  Fixed bug
							in birdTCPIPClearBuffer().

		07/15/99	bel		Modified birdRS232WakeUp() and birdISAWakeUp() to
							put the transmitter into pulsed mode on startup.

		08/17/99	bel		Added birdTDFWakeUp(), birdTDFStartStream(),
							birdTDFGetReading(), birdTDFGetMostRecentReading(),
							birdTDFReadingReady(), and birdTDFStopStream()
							for controlling the new three-degree-of-freedom 
							orientation sensor.

		10/09/99	bel		Added birdMB2WakeUp() for communication with the
							MiniBird2 device.
...
/*
	birdTDFWakeUp		Bird wakeup, TDF mode
	...
	Remarks:            Wakes up a single bird in TDF mode.  "nGroupID" 
						can be any number between 0 and BIRD_MAX_GROUP_ID 
						that the user wants to associate with this bird.
						"wComport" is the number of the comport attached to 
						the bird (e.g., COM1 = 1, COM2 = 2, etc.)
						"dwReadTimeout" is the maximum time, in msecs, that 
						the application will take when trying to receive a 
						character.  "dwWriteTimeout" is the maximum time, in 
						msecs, that the application will take when trying 
						to transmit a character.
*/
BOOL DLLEXPORT birdTDFWakeUp(int nGroupID, WORD wComport, DWORD dwReadTimeout, DWORD dwWriteTimeout);
...
/*
	...
	Remarks:            Starts acquisition of a single frame of bird data.
						...
						NOTE: In ISA mode, data is collected from internal birds
							  only, and stand-alone mode is not supported.  If you
							  are running in stand-alone mode, or need to collect
							  data from external birds, use birdStartReading() 
							  instead of this routine to manually get readings 
							  from each bird.
...
/*
	birdTDFStartStream	Start stream, TDF mode
	...
	Remarks:            Starts streaming of bird data in TDF mode.  
						"nGroupID" is the group identifier that was passed 
						to birdTDFWakeUp().  The readings are retrieved 
						by calling birdTDFGetReading().
*/
BOOL DLLEXPORT birdTDFStartStream(int nGroupID);

/*
	birdTDFGetReading	Get reading, TDF mode
	...
	Remarks:            Gets the next bird reading during streamed data
						acquisition.  "nGroupID" is the group identifier that 
						was passed to birdTDFWakeUp().  "pfReading" points to
						an array of floats into which the reading is copied.
						If "nDataFormat" equals 1 (euler angles), then 
						"pfReading" must point to an array of three floats,
						corresponding to azimuth, elevation, and roll.  If
						"nDataFormat" equals 2 (rotation matrix), then
						"pfReading" must point to an array of nine floats,
						corresponding to a 3x3 rotation matrix.
*/
BOOL DLLEXPORT birdTDFGetReading(int nGroupID, float *pfReading, int nDataFormat);

/*
	birdTDFGetMostRecentReading	Get most recent reading, TDF mode
	...
	Remarks:            Gets the most recent bird reading during streamed data
						acquisition.  "nGroupID" is the group identifier that 
						was passed to birdTDFWakeUp().  "pfReading" points to
						an array of floats into which the reading is copied.
						If "nDataFormat" equals 1 (euler angles), then 
						...
						corresponding to a 3x3 rotation matrix.
*/
BOOL DLLEXPORT birdTDFGetMostRecentReading(int nGroupID, float *pfReading, int nDataFormat);

/*
	birdTDFReadingReady	Reading ready?

	Parameters Passed:  int nGroupID

	Return Value:       TRUE if a new reading is ready
						FALSE otherwise

	Remarks:            Determines if a new bird reading is ready during 
						streamed data acquisition.  "nGroupID" is the group 
						identifier that was passed to birdTDFWakeUp().  
*/
BOOL DLLEXPORT birdTDFReadingReady(int nGroupID);

/*
	birdTDFStopStream	Stop stream, TDF mode
	...
	Remarks:            Stops streaming of bird data in TDF mode.  "nGroupID" is
						the group identifier that was passed to birdTDFWakeUp().
*/
BOOL DLLEXPORT birdTDFStopStream(int nGroupID);

/*
	birdTDFSendCommand  Send command, TDF mode
	...
	Remarks:            Sends a generic command in TDF mode.  "nGroupID" is
						the group identifier that was passed to birdWakeUp().
						The command bytes are passed in "pbuffer", and the 
						number of bytes is passed in "wNumBytes".
*/
BOOL DLLEXPORT birdTDFSendCommand(int nGroupID, void *pbuffer, WORD wNumBytes);

/*
	birdTDFGetResponse  Get response, TDF mode
	...
	Remarks:            Gets a generic response in TDF mode.  "nGroupID" is
						the group identifier that was passed to birdWakeUp().
						The response bytes are passed in "pbuffer", and the 
						number of bytes is passed in "wNumBytes".
*/
BOOL DLLEXPORT birdTDFGetResponse(int nGroupID, void *pbuffer, WORD wNumBytes);

/*
	birdTDFClearBuffer  Clear buffer, TDF mode
	...
	Remarks:            Clears the TDF receiver buffer.  "nGroupID" is the
						group identifier that was passed to birdWakeUp().
*/
BOOL DLLEXPORT birdTDFClearBuffer(int nGroupID);

/*
	birdTDFResynch		Resynchronize, TDF mode
	...
	Remarks:            Resynchronizes the TDF data stream during 
						streamed frame acquisition.  "nGroupID" is the 
						group identifier that was passed to birdWakeUp().
*/
BOOL DLLEXPORT birdTDFResynch(int nGroupID);

/*
	birdTDFReset		Reset, TDF mode
	...
	Remarks:            Resets a group of birds by toggling the TDF RTS 
						line.  "nGroupID" is the group identifier that was 
						passed to birdWakeUp().
*/
BOOL DLLEXPORT birdTDFReset(int nGroupID);
...
/*
	birdGetDllVersion	Get DLL software version string

	Parameters Passed:  char *szRev  version string

	Return Value:       void

	Remarks:            Gets the current DLL version string.
