...
//  Information about the matrix...
    ...
    BOOL isquare( void );                            // Is the matrix square (rows == columns)?
    ...
    double norm( void );                         // Norm of a vector.
...
//  Extraction operators...
  ...
  ? problem  matrix operator()( matrix &A );              // Sub-matrix extraction by vector.    [r1 c1 r2 c2]
...
//  double matrix::operator double();
    ...
    // for deleteion friend matrix operator~(matrix&);                 // A = ~B;
                     */
    ...
    void normclamp( double minimum, double maximum );
    void normclamp( double absolute );
};
