...
//  double matrix::operator double();
    ...
    void   operator*=( matrix & );                    // A *= B;
    void   operator*=( double   );                    // A *= (double)d;
    void   operator*=( float    );                    // A *= (float)f;
    void   operator*=( int      );                    // A *= (int)i;
   ...
   // Concatenation columns...
    ...
    friend matrix operator|( double, matrix & );      // C = (double)a | B;
    friend matrix operator|( float,  matrix & );      // C = (float)a | B;
    friend matrix operator|( int,    matrix & );      // C = (int)a | B;
...
#define MATRIX_LINEAR_MAX       256         // Maximum number of elements in linear array.

#define MATRIX_BY_COL           0           // Represent matrix data column-by-column.
#define MATRIX_BY_ROW           1           // Represent matrix data row-by-row.
#define MATRIX_BY_DEFAULT       MATRIX_BY_COL
...
void    martix_incspace( matrix &mtx, double a, double b, double step );
void    martix_incspace( matrix &mtx, int a, int b, int step );
void    martix_incspace( matrix &mtx, int a, int b );
...
matrix  perpend( matrix &A, matrix &B );              // Vector product normalized of A and B...
...
matrix  sortcols( matrix &A, int column );
...
#define RANDOM_GAUSSIAN_POINTS 200000
...
#define MATRIX_ConstructionBefore()    if( MATRIX_TimingFlag ) MATRIX_TimingConstruction.Before();
#define MATRIX_ConstructionAfter()     if( MATRIX_TimingFlag ) MATRIX_TimingConstruction.After();
#define MATRIX_DestructionBefore()     if( MATRIX_TimingFlag ) MATRIX_TimingDestruction.Before();
#define MATRIX_DestructionAfter()      if( MATRIX_TimingFlag ) MATRIX_TimingDestruction.After();
